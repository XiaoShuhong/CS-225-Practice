...
class KDTree
{
  ...
  private:
    ...
    void makeNodeFromVec(vector<Point<Dim>> newPoints, int curDim, KDTreeNode* myNode);
    Point<Dim> findNearestNeighborHelper(KDTreeNode* myNode,const Point<Dim>& query, int curDim) const;
};
